function output = sysf_piecewise_const_stretch_abs_b_318(input_mode,pathnames)
...
%     curvdef = 'a1*heaviside(-s)+a2*heaviside(s)';
    ...
    % no decimal issues
    curvdef_name = ['piecewise_const_stretch_abs_b_',num2str(stretch_const*1000)];
	...
	switch input_mode
		...
		case 'name'
			...
			output = ['Piecewise Stretchable Snake, b = ',num2str(stretch_const),', abs labmda func']; % Display name
...
%                 pathnames.sysplotterpath,pathnames.syspath)
            ...
            make_curvdef_piecewise_abs_stretch(stretch_const,pathnames.sysplotterpath,pathnames.syspath)
