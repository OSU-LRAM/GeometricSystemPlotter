function output = sysf_serpenoid_extendable_abs_318(input_mode,pathnames)
    ...
    stretch_const = 3.180000e-01;
    stretchdef = [num2str(stretch_const),'*abs(a1*cos(2*pi*s)+a2*sin(2*pi*s))+1'];
    curvdef = 'a1*cos(2*pi*s)+a2*sin(2*pi*s)';
    % creates file name with b constant 1000 times the actual, so there's
    % no decimal issues
    curvdef_name = ['serpenoid_stretch_abs_b_',num2str(stretch_const*1000)]; 
	%%%%%%%
	...
	switch input_mode
		...
		case 'name'
			...
			output = ['Serpenoid Stretchable Snake, b = ',num2str(stretch_const),', abs lambda func']; % Display name
		...
		case 'initialize'
            ...
            make_curvdef_stretch({curvdef,stretchdef},{'a1','a2'},...
                ['serpenoid_stretch_abs_b_',num2str(stretch_const*1000)],[1 1 1 1 1 1],...
            ...
            % s.grid_range = [-1,1,-1,1]*ceil((6/0.318)*stretch_const + 12);
