function output = sysf_piecewise_peristaltic_abs_b_30(input_mode,pathnames)
...
%     curvdef = 'a1*heaviside(-s)+a2*heaviside(s)';
    ...
    % no decimal issues
    curvdef_name = 'piecewise_peristaltic_abs_b_30';
                   %curv_piecewise_peristaltic_abs_b_30
	...
	switch input_mode
		...
		case 'name'
			...
			output = ['Piecewise Peristaltic Snake, b = ',num2str(stretch_const),', abs']; % Display name
...
%             s.grid_range = [-1,1,-1,1]*ceil((3/0.1)*stretch_const + 12);
			...
			s.metric = @(x,y) LowRE_dissipation_metric_from_stretchable_curvature...
				(curvdef_fun,[x;y],0.02,1,2);  % @(x,y) eye(2);%
...
function [Ar]=Conn_num(a1,a2,curvdef_fun)
		...
		Ar =  LowRE_local_connection_from_stretchable_curvature(curvdef_fun,[a1;a2],0.02,1,2);
