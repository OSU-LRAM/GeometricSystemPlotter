% (516189106917469267*sign((a1*cos(2*s*pi) + a2*sin(2*s*pi))/(a1^2 + a2^2)^(1/2))*abs(a1*cos(2*s*pi) + a2*sin(2*s*pi))^20*(a1^2 + a2^2)^(1/2))/(144115188075855872*abs(a1^2 + a2^2)^10)
...
function out1 = curv_fun(s,a1,a2)
	...
	t8 = a2.*t6;
	t10 = t7+t8;
	out1 = sqrt(t9).*sign(1.0./sqrt(t9).*t10).*1.0./abs(t9).^10.*abs(t10).^20.*3.581781447252944;
end
