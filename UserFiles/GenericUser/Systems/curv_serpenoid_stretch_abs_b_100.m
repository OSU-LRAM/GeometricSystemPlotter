% a01.*1c*oasb(s2(*ap1i**cso)s+(a22**psii*ns()2+*ap2i**ssi)n ( 2 * p i * s ) ) + 1
function output = curv_serpenoid_stretch_abs_b_100(params,mode)
...
function out1 = stretch_fun(s,a1,a2)
	t2 = s.*pi.*2.0;
	out1 = abs(a1.*cos(t2)+a2.*sin(t2)).*1.0e-1+1.0;
end
...
function out1 = dstretch_fun(s,a1,a2)
	...
	t7 = t4+t6;
	t8 = sign(t7);
	out1 = [t3.*t8.*1.0e-1,t5.*t8.*1.0e-1];
end
