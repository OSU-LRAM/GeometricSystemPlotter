function salp = threeLinkSalp_min(inputmode)

    %%%%%%
    %% Body geometry
    swimmerName = 'Three-Link Minimal Salp Example';
    ...
    switch inputmode
        ...
        case 'getSwimmerSystem'
            ...
            s.geometry.linklengths = ones(1,3);
            s.geometry.length = 1;
            ...
            s.geometry.thrusters.thrusterArcLengths = .5*ones(1,s.geometry.nLinks);
            s.geometry.thrusters.nThrusters = numel(s.geometry.thrusters.thrusterArcLengths);
        
            thrusterMag = pi/6;
            thrusterSign = 2*mod(s.geometry.thrusters.linkLocations,2)-1;
