function s = evaluate_inertia_tensors_in_system_file(s,component_list,zoom_list,destination)
    ...
    for i = 1:size(zoom_list,1)
        ...
        for j = 1:length(component_list)
            ...
            if isfield(s,component_list{j})
               ...
               if strcmpi(component_list{j},'A')
                   [s.A,s.J,s.J_full,s.M_full] = evaluate_inertia_local_connection(s.(component_list{j}),a,ignore_singularity_warning);
