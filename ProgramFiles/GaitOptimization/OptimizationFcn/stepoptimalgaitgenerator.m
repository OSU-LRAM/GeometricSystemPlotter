function y=stepoptimalgaitgenerator(s,dimension,npoints,a,lb,ub,stretch,direction,costfunction,handles)
%%%%%%%%%%%%%%
...
% direction: Direction to optimize travel: 1=x,2=y,3=theta,4=steering gait
...
global bestCost bestDisp bestEff stepOptimalGaits currentDisp Eff;
bestEff = 0;
currentDisp = [0;0;0;0];
...
% If it is optimization for a steering gait, set the initial gait as the
% x direction optimal gait.
if direction == 4
    initdir = 1;
else
    initdir = direction;
...
constraint_function = @(y) nonlcon(y,s,npoints,dimension,lb,ub,initdir);
