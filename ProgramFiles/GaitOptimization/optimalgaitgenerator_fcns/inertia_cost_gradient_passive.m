...
%   ode45. Other values will result in error.
    ...
    if strcmpi(EvaluationMethod,'discrete')
        ...
        if isfield(s,'actuatormetric')
            ...
            actuatormetric = eye(size(metrics{1}));
        end
                
        switch s.costfunction
            case 'torque'
                for k = 1:length(t_pts)
                    del_cost = inertia_gradient_helper(t_pts(k),[],s,gait,grad_alpha,grad_alphadot,grad_alphaddot,metrics{k},dMs{k},ddMs{k},actuatormetric);
                    cost_grad = cost_grad + reshape(del_cost,size(cost_grad)).*del_t;
                end
            case 'mechanical power'
                for k = 1:length(t_pts)
                    del_cost = mechpower_gradient_helper(t_pts(k),[],s,gait,grad_alpha,grad_alphadot,grad_alphaddot,metrics{k},dMs{k},ddMs{k},actuatormetric);
                    cost_grad = cost_grad + reshape(del_cost,size(cost_grad)).*del_t;
                end
    ...
    elseif strcmpi(EvaluationMethod,'ode45')
        switch s.costfunction
            case 'torque'
                sol = ode45(@(t,y) inertia_gradient_helper(t,y,s,gait,grad_alpha,grad_alphadot,grad_alphaddot,actuatormetric),tspan,cost_grad);
            case 'mechanical power'
                sol = ode45(@(t,y) mechpower_gradient_helper(t,y,s,gait,grad_alpha,grad_alphadot,grad_alphaddot,actuatormetric),tspan,cost_grad);
            case 'covariant acceleration'
