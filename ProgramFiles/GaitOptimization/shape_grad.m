...
for i = 1:num_coeffs 
    for j = 1:dim
        ...
        if i == 1
            grad_alpha{i,j} = @(t) [zeros(numel(t),j-1), ones(numel(t),1) , zeros(numel(t),dim-j)];
            grad_alphadot{i,j} = @(t) zeros(numel(t),dim);
            grad_alphaddot{i,j} = @(t) zeros(numel(t),dim);
            continue
        elseif i == num_coeffs % Partial w.r.t. frequency
            grad_alpha{i,j} = @(t) [zeros(numel(t),j-1), t.*(-y(2,j)*sin(w(j)*t) + y(3,j)*cos(w(j)*t) - ...
                                               2*y(4,j)*sin(2*w(j)*t) + 2*y(5,j)*cos(2*w(j)*t) - ...
                                               3*y(6,j)*sin(3*w(j)*t) + 3*y(7,j)*cos(3*w(j)*t) - ...
                                               4*y(8,j)*sin(4*w(j)*t) + 4*y(9,j)*cos(4*w(j)*t)), ...
                                    zeros(numel(t),dim-j)];
            
            grad_alphadot{i,j} =  @(t) [zeros(numel(t),j-1), -y(2,j)*(w(j)*t.*cos(w(j)*t)+sin(w(j)*t)) + y(3,j)*(-w(j)*t.*(sin(w(j)*t))+cos(w(j)*t)) + ...
            ...
            grad_alphaddot{i,j} =   @(t) [zeros(numel(t),j-1), -y(2,j)*w(j)*(-t.*(w(j)*sin(w(j)*t))+2*cos(w(j)*t)) - y(3,j)*w(j)*(t.*(w(j)*cos(w(j)*t))+2*sin(w(j)*t)) + ...
        ...
        grad_alpha{i,j} = @(t) [zeros(numel(t),j-1), trig(mult*w(j)*t), zeros(numel(t),dim-j)];
        ...
        grad_alphadot{i,j} = @(t) [zeros(numel(t),j-1), (-1)^(i-1)*mult*w(j)*trig(mult*w(j)*t), zeros(numel(t),dim-j)];
        ...
        grad_alphaddot{i,j} = @(t) [zeros(numel(t),j-1), -mult^2*w(j)^2*trig(mult*w(j)*t), zeros(numel(t),dim-j)];
    end
