...
function s = optimize_coordinate_choice(s)
    ...
    if isfield(s,'noop') && s.noop
        % TODO feed dummy values instead of calculating a new coordinate choice
        s.vecfield.eval.content.Avec_optimized = vecfield_eval;
        s.B_optimized.eval.Beta = {0;0;0};
        s.B_optimized.eval.gradBeta = [0;0;0];
