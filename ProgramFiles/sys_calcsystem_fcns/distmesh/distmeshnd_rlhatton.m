function [p,t]=distmeshnd_rlhatton(fdist,fh,h,box,fix,display_fig_handle,varargin)
...
while 1
  ...
  while any(ix)
	  ...
	  gradd = gradd./repmat((sqrt(sum(gradd.^2,2))),1,dim); % normalize gradient vector
	  p(ix,:)=p(ix,:)-(d(ix)*(ones(1,dim)*kickfactor).*gradd);
	  d=feval(fdist,p,varargin{:});
	  ix=d>0;
