function [Y, R, E] = isomap_fast(D, n_fcn, n_size, options); 

% ISOMAP_FAST   Computes Isomap embedding using an advanced version of
...
size_comps = sum((repmat(firsts,n_comps,1)==((1:n_comps)'*ones(1,N)))');    %% size of each connected component
...
Y.index = setdiff(Y.index,find(isinf(min(D)))); %% prune points that don't connect to any landmarks
N = length(Y.index); 
...
if (nl==N)
     subB = center(D,N,N);
     [vec, val] = eigs(subB, max(dims), 'LR', opt); 
else
     subB = center(D,N,nl);
...
for di = 1:length(dims)
     ...
     % this actually computes matrixnorm of distance matrices (centered)
