function [B,h,J, J_full] = wheeled_chain(geometry, jointangles, display)
...
%       selected baseframe, with the first link held fixed.
    ...
    B = cell(N_links,1);      % Cell Array to hold points on wheels
    W = cell(N_links,1);      % Cell Array to hold points on wheels
    ...
    wheel_scale_length = .3;
    wheel_scale_width = .6;

    %placement of wheels relative to links
    wheel_outboard = 1.1*width*[1;-1];
    ...
    for idx = 1:N_links
        ...
        % get the points on the ellipse-ended rectangle for link
        C = squashed_rectangle(length,width,cap_width,points);
        
        % get the points on the ellipse-ended rectangle for wheels
        X = diag([wheel_scale_length,wheel_scale_width,1])*C;
        X1 = vec_to_mat_SE2([0 wheel_outboard(1) 0]) * X;
        X2 = vec_to_mat_SE2([0 wheel_outboard(2) 0]) * X;
        
        % Transform rectangle to link position
        C = g(:,:,idx) * C;
        
        % Transform wheels to their positions
        W1 = g(:,:,idx) * X1;
        W2 = g(:,:,idx) * X2;
        
        % Put C into the B cell array
        B{idx} = C;
        
        % Put W1 and W2 into the W cell array
        W(2*idx-1:2*idx) = {W1;W2};
        
        
        %%%
        % Create ellipse-ended rectangles for wheels
        
    end
    
    % Return the links and wheels as a cell array
    B = {B, W};
