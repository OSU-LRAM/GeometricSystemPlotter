...
% 	end
	...
	switch imageformat{i}
		...
		case 'vector'
			...
			%%%%%%%%%
			% Currently, not extracting bounding box, but using an
			% experimentally-generated value that takes into account the
			% shift in coordinates imposed by ghostscript when flattening
			% out the fonts in the axis file
...
% 			fclose(fid);
			...
			filename = [num2str(i) '_flat.eps'];
			fid = fopen(fullfile(dirname,filename));

			% loop down strings in file
			bbox = [2 0 574 433]; %These are experimentally-generated values, and account for ghostscript moving the content during epsflatten
			while ~feof(fid)
				...
				if strncmp(s,'%%Bounding',10)
					s = ['%%BoundingBox: ' num2str(bbox - [0 0 0 0])];
					s2 = ['%%HiResBoundingBox: ' num2str(bbox - [0 0 0 0])];
					fprintf(fido,'%s\n',s2);
					
				elseif strncmp(s,'%%HiResBounding',10)
					s = '%%';
				end
			...
			unix(['cd ~/Temp/painted_layer/; epstopdf --hires '  filename '; mv ' regexprep(filename,'.eps','.pdf') ' ' num2str(i) '.pdf']);
			
		case 'raster'
			
 			unix(['cd ~/Temp/painted_layer/; convert ' num2str(i) '.png -transparent \#00001E ' num2str(i) '.pdf']);
...
%%%%%%%
% send file to destination, cropping whitespace
switch destination(end-2:end)
	
	case 'pdf'
		
		unix(['cd ~/Temp/painted_layer/; pdfcrop combinedfile.pdf ' destination]);
