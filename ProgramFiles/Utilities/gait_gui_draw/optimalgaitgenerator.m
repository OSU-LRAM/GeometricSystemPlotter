function y=optimalgaitgenerator(s,dimension,npoints)
...
ub=2.2*ones(200,1);
nonlcon=[];
options = optimoptions('fmincon','SpecifyObjectiveGradient',true,'Display','iter','Algorithm','active-set','GradObj','on','TolX',10^-6,'TolFun',10^-6,'MaxIter',4000,'MaxFunEvals',20000,'FiniteDifferenceStepSize',10^-8);
...
for i=1:1:n
    ...
    for j=1:dimension*(dimension-1)/2
        height(i,j)=interpn(interpstateheight{:},s.height_optimized_corrected{1,j},yvalues{:});
    end
    ...
    for j=1:1:dimension
        for k=1:1:dimension
            metric{i}(j,k)=interpn(interpmetricgrid{:},s.metricfield.metric_eval.content.metric{j,k},yvalues{:});
        end
    end
% metric{i}=eye(2);
    ...
    for l=1:1:dimension
        for m=1:1:dimension
            ...
            for j=1:1:dimension
                for k=1:1:dimension
                    metricgrad{i,l}(j,k)=(interpn(interpmetricgrid{:}, s.metricfield.metric_eval.content.metric{j,k}, yvalues2{:})-interpn(interpmetricgrid{:}, s.metricfield.metric_eval.content.metric{j,k}, yvalues1{:}))/(2*afactor);
                end
            end
...
if invert == 0
    ...
    totaljacobian=jacobiandisp-(((lineint)*jacobianstroke)/(1*totalstroke))/1+0*jacobianeqi;
    dsdw=jacobiandisp;
    dy=dsdw(:);
else
    %totaljacobiantemp=jacobiandisp/totalstroke-(((lineint)*jacobianstroke)/(1*totalstroke^2))/1+1*jacobianeqi;
    totaljacobiantemp=jacobiandisp-(((lineint)*jacobianstroke)/(1*totalstroke))/1+0*jacobianeqi;
    for i=1:n
