function [A,B,t,p] = fseriesdemo(f,b,NN,upsamplefactor)
...
% Upsample the x vector
x=[x,2*pi];
x_up = interp1(linspace(0,1,length(x)),x,linspace(0,1,length(x)*upsamplefactor));

p = app1(x_up,A0,AN,BN,T);
% Now on to the plotting.
xx = linspace(eps,x_up(end),b*100);   
xx(end) = [];   
fun = f(xx); 
hand = plot(xx,fun,'r',x_up,p,'b');   
