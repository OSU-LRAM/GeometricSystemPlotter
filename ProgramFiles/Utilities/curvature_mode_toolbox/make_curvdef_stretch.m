function make_curvdef_stretch(curv_fun_cell,paramlist,name,attempt_analytic,sysplotterpath,syspath)
...
% Function for stretch along backbone
if attempt_analytic(5)
    stretch_fun = sym(curv_fun_cell{2});
    flist = [flist,{stretch_fun}];
else
    flist = [flist,{[]}];
end

% Take derivative of stretch
if attempt_analytic(6)
%     syms s
%     dstretch_fun = diff(stretch_fun,s);
    dstretch_fun = jacobian(stretch_fun,sym(paramlist));
%     clear s
    flist = [flist,{dstretch_fun}];
else
    flist = [flist,{[]}];
end
...
fnamelist = {'curv_fun','int_curv_ds_fun','d_curv_dp_fun','int_d_curv_dp_ds_fun','stretch_fun','dstretch_fun'};
    ...
    if ~no_analytical
        ...
        switch fnamelist{idx}
            ...
            case 'stretch_fun'
                ...
                stretch_call = {'output = @(s) stretch_fun(s,params{:});'};
            
            case 'dstretch_fun'
                
                dstretch_call = {'output = @(s) dstretch_fun(s,params{:});'};
        ...
        switch fnamelist{idx} 
            ...
            % need to add cases for stretch and d_stretch. 
...
while ~feof(fidi)
    ...
    switch strtrim(s)
        ...
        case 'AA_stretch_call'
            
            s = sprintf('\t\t%s\n',stretch_call{:});
            
        case 'AA_dstretch_call'
