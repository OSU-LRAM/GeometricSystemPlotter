...
function make_curvdef_piecewise_stretch(b,sysplotterpath,syspath)

% b = 0.3;
% template file, in same directory
template = 'curvdef_stretch_piecewise_template.txt'; % For continuous curvature piecewise
...
for ind = 1:numel(b)
    ...
    fido = fopen(fullfile(syspath,['curv_piecewise_const_stretch_b_',num2str(b(ind)*1000),'.m']),'w');
