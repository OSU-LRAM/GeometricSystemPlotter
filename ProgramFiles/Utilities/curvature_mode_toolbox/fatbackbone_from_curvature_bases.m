function [B,h] = fatbackbone_from_curvature_bases(kappa_basis_input,r,L,width,orientation)
    ...
    switch orientation
        ...
        case 'from-sys'
            ...
            % Note: tol was too small so I tweaked it to be just a little
            % larger...
            ...
            % note for above: sometimes, it will instead be TWO values...
            % what do we do then??
            
            % THIS IS IT!! FIXED THE BUG!!
            % Should continue to work through this code in notebook to be
            % sure there aren't more to squash!!
            [k,j]=find(abs(s.grid.eval{2}(1,:)-r(2))<=tol); % find the indicies where a1 AND a2 are as specified
