...
function make_curvdef_piecewise_stretch_abs_Lrms(b,sysplotterpath,syspath)

% b = 0.3;
% template file, in same directory
template = 'curvdef_stretch_piecewise_abs_template_Lrms.txt'; % For continuous curvature piecewise
...
for ind = 1:numel(b)
    ...
    fido = fopen(fullfile(syspath,['curv_piecewise_abs_L_',num2str(b(ind)*1000),'.m']),'w');
