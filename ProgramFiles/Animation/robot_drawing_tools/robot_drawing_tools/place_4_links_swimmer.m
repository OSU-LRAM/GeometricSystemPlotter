...
linkpos = [-2*L*cos(-alpha_2/2)-L*cos(-alpha_1-alpha_2/2),-L*cos(-alpha_2/2), L*cos(alpha_2/2),2*L*cos(alpha_2/2)+L*cos(alpha_3+alpha_2/2);
    -2*L*sin(-alpha_2/2)-L*sin(-alpha_1-alpha_2/2),-L*sin(-alpha_2/2), L*sin(alpha_2/2), 2*L*sin(alpha_2/2)+L*sin(alpha_3+alpha_2/2);
     -alpha_1-alpha_2/2,-alpha_2/2,alpha_2/2,alpha_2/2+alpha_3 ];
...
switch coords
	...
	case 'optimized'
		...
		beta_x = interpn(s.grid.eval{1}, s.grid.eval{2},s.grid.eval{3}, s.B_optimized.eval.Beta{1},alpha_1,alpha_2,alpha_3);
		beta_y = interpn(s.grid.eval{1}, s.grid.eval{2},s.grid.eval{3}, s.B_optimized.eval.Beta{2},alpha_1,alpha_2,alpha_3);
		beta_theta = interpn(s.grid.eval{1}, s.grid.eval{2},s.grid.eval{3}, s.B_optimized.eval.Beta{3},alpha_1,alpha_2,alpha_3);
