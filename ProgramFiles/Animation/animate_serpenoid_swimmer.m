function animate_serpenoid_swimmer(export,info_needed)
	...
	% Declare timing
	timing.duration = 8; % three second animation
...
function h = create_elements
	...
	%Create the robot
	h.robot = create_serpenoid(h.ax,data_source,system_name);
...
function frame_info = execute_gait(frame_info,tau)
	...
	frame_info{irobot}.robot = place_serpenoid(frame_info{irobot}.robot,config,'optimized');
