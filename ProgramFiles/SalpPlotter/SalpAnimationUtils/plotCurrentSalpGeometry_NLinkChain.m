function plotCurrentSalpGeometry_NLinkChain(ax,base_anim_geom,salp,pose,forcedef,t)
    
    %Get transform that moves all salp elements to current pose in world frame
    poseTransform = vec_to_mat_SE2(pose);

    %Plot each link
    for link_ind = 1:salp.geometry.nLinks
        currentLink = base_anim_geom.links.points{link_ind};
        inWorldLink = poseTransform*currentLink;
        fill(ax,inWorldLink(1,:),inWorldLink(2,:),base_anim_geom.links.color);
    end
